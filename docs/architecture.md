...
`gym_memberships` doesn't fit the "gated by `app.current_gym_id`" pattern used elsewhere, because it's the table that *produces* `gymId` in the first place. Its policy is instead: a row is visible if `user_id = current_setting('app.current_user_id', true)` **or** `gym_id = current_setting('app.current_gym_id', true)` **or** the caller is a platform admin. The first branch is what makes the bootstrap lookup in `lib/server/auth.ts` safe without needing a `gymId` yet.

Policy-writing rule to keep this from becoming circular: `gym_memberships`'s own policies never reference `users` — they only read the `app.*` settings. `users`' `SELECT` policy is allowed to reference `gym_memberships` (a Gym Admin may read the email of a user with a `gym_memberships` row in their own gym, added in Phase 2 — see `docs/decisions.md`), which makes the dependency one-directional (`users` → `gym_memberships`), not a cycle. The rule is "no cycle," not "no reference at all" — Phase 1's original wording of this section stated the stricter version as a simplification, not a security requirement, and Phase 2 needed to relax it once a real, tested need for Gym-Admin-scoped user visibility appeared.
